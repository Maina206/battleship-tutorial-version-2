...
function HitAttackSimulation() {
  ...
  const isShipPresentAndNotAttacked = (row, col) => {
    const isAttacked = attackedCells.some(
  ...
  return (
    <div className="container" id="step5">
      ...
      <p>
        In the hit scenario, players take turns attempting to strike their
        opponent's ships. When a player selects a coordinate to attack, the game
        checks if there is a ship located at that coordinate. If there is a ship
        present, the attack is considered a hit, and the corresponding part of
        the ship is marked. A visual indication (such as a red color) shows that
        the attack was successful. Conversely, if no ship is present, the attack
        is marked as a miss (typically shown in gray). The game continues until
        all of one player's ships are sunk, declaring the other player as the
        winner. Below this explanation there are two grids one for "player 1's
        battleship grid board "and the other for "player 2's battleship pin
        board", Below is a simulation that shows a hit scenario;
      </p>
      <div className="sub-step">
        {/* Player 1 Grid with Ships */}
        <div className="players">
          <div>
            <h4>Player 1 ship board</h4>
            <Grid
              elementID="player1"
              onCellClick={() => {}}
